...
function App() {
  ...
  const [searchData, setSearchData] = useState(JSON.parse(sessionStorage.getItem("searchData")) || {});
  const [hoaxData, setHoaxData] = useState(sessionStorage.getItem("hoaxData") || {});
  const [hateData, setHateData] = useState(sessionStorage.getItem("hateData") || {});
  ...
  return (
    <Routes>
      ...
      <Route path="/cekberita" element={<CekBerita setSearch={setSearch} />} />
      <Route path="/search" element={<SearchPage searchData={searchData} setSearch={setSearch} hoaxData={hoaxData} hateData={hateData} />} />
