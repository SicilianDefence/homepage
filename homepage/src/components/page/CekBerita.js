...
const CekBerita = ({ setSearch }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (/^[a-zA-Z0-9].*/.test(term) || /^[a-zA-Z0-9]+[" "]/.test(term) || /^[" "]+[a-zA-Z0-9]/.test(term)) {
      setSearch(term.trim());
    }
  };
    ...
    <div className="mySecondBG">
      ...
      <Container>
        <div className="cek-body">
          <div className="page2-title">Verifikasi Mudah: Cek Kebenaran Beritamu!</div>
          <div className="page2-content">
            <div className="line1-content ">"Menyaring Kebenaran: Cek Hoax, Analisis Berita, dan Pemetaan Sentimen</div>
            <div className="line2-content">Pemilu untuk Informasi yang Akurat dan Terpercaya"</div>
          </div>
          <div className="search-box d-flex py-2 align-items-center ">
            <button type="button" className="btn search" onClick={() => handleSearch()}>
              <i className="fa fa-search"></i>
            </button>
            <form className="form-search text-form" onSubmit={(e) => handleSubmit(e)}>
              <input type="text" name="term" id="term" className="search-google" value={term} onChange={(e) => setTerm(e.target.value)} />
              {term ? (
